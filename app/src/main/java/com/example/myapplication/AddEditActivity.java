...
public class AddEditActivity extends AppCompatActivity {
    ...
    private boolean validerFormulaire(String titre, String auteur, String isbn) {
        boolean ok = true;
        if (TextUtils.isEmpty(titre)) { etTitre.setError("Le titre est obligatoire"); ok = false; }
        if (TextUtils.isEmpty(auteur)) { etAuteur.setError("L'auteur est obligatoire"); ok = false; }
        if (!TextUtils.isEmpty(isbn) && isbn.length() < 10) {
            etIsbn.setError("L'ISBN doit contenir au moins 10 caracteres"); ok = false;
        }
        return ok;
    }
